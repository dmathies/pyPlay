...
class ArtNetClient:
  ...
  def set_config(self, config: dict, rdm_devices=None) -> None:
    self.config = config
  ...
  def register_rdm_config_callback(self,
      callback: Callable[[int, int, dict], None]) -> None:
  ...
  def handle_poll_request(self, op_code: OpCode, ip: str, port: int,
      reply: dict[str, Any]) -> None:
    self.artnet.send_poll_reply(ip, port, self.config)

  def handle_tod_request(self, op_code: OpCode, ip: str, port: int,
      reply: dict[str, Any]) -> None:
  ...
  def RdmGetDeviceInfo(self, uid, data: bytearray = b"") -> bytes:
    device = self.get_device_by_id(uid)
    if device is None:
      return b""

    current_personality_id = device["parameters"][RdmParameterID.RdmParamDmxPersonality]
    current_personality = device.get("dmx_personalities", [])[current_personality_id-1]
    ...
    packet = (
        ...
        + struct.pack("B", len(device.get("dmx_personalities", [])) )  # DMX Personality count
        + struct.pack(">H", device["parameters"][
      RdmParameterID.RdmParamDmxStartAddress])
        + struct.pack(">H",
                      device["DMX Sub Device Count"])  # DMX Sub Device Count
        + struct.pack("B", device["sensor_count"])  # Sensor Count
    )
    return packet

  def RdmGetDeviceParam(self, uid, param_id, data: bytearray = b"") -> bytes:
    device = self.get_device_by_id(uid)
  ...
  def RdmSetDeviceParam(self, uid, param_id, data: bytearray = b"") -> bytes:
    device = self.get_device_by_id(uid)
    custom_parameter = self.get_param_by_id(uid, param_id)
    ...
    if custom_parameter is not None:
      ...
      if data_type is not None:
        ...
        try:
          if index is not None:
            if isinstance(device["parameters"][param_id][index_val],
                          (list, tuple)):
  ...
  def RdmParamPersonality(self, uid, data: bytearray = b"") -> bytes:
    device = self.get_device_by_id(uid)
    packet = []
    personality_count = len(device.get("dmx_personalities", []))
    packet.append(struct.pack("B", device["parameters"][RdmParameterID.RdmParamDmxPersonality]))
    packet.append(struct.pack("B", personality_count))

    return b"".join(packet)

  def RdmParamPersonalityDescription(self, uid, data: bytearray = b"") -> bytes:
    device = self.get_device_by_id(uid)
    ...
    try:
      ...
      packet.append(struct.pack("32s", bytes(personality.get("name", ""), "ascii")))
  ...
  def RdmParamSlotInfo(self, uid, data: bytearray = b"") -> bytes:
    device = self.get_device_by_id(uid)
    current_personality_id = device["parameters"][RdmParameterID.RdmParamDmxPersonality]
    current_personality = device.get("dmx_personalities", [])[current_personality_id-1]
  ...
  def RdmParamDefaultSlotValue(self, uid, data: bytearray = b"") -> bytes:
    device = self.get_device_by_id(uid)
    current_personality_id = device["parameters"][RdmParameterID.RdmParamDmxPersonality]
    current_personality = device.get("dmx_personalities", [])[current_personality_id-1]
  ...
  def RdmParamSlotDescription(self, uid, data: bytearray = b"") -> bytes:
    device = self.get_device_by_id(uid)
    current_personality_id = device["parameters"][RdmParameterID.RdmParamDmxPersonality]
    current_personality = device.get("dmx_personalities", [])[current_personality_id-1]
  ...
  def RdmParamSupportedParameters(self, uid, data: bytearray = b"") -> bytes:
    device = self.get_device_by_id(uid)
    packet = []
  ...
  def RdmParamParameterDescription(self, uid, data: bytearray = b"") -> bytes:
  ...
  def handle_rdm_request(self, op_code: OpCode, ip: str, port: int,
      packet: bytes) -> None:
    # print(f"Received {op_code.name} (request) from {ip}:{port}")

    request: dict = parse_rdm(bytes(packet))

    # for k, v in request.items():
    #   print(f"\t{k} = {v}")

    command = request.get('RdmCommand')
    parameter_id = request.get('RdmParameterId')
    ...
    if command == RdmCommandClass.RdmGetCommand:
      ...
      if getter is not None:
        reply = getter(request.get('RdmDestUID'), request.get('RdmParameterData'))
      else:
        reply = self.RdmGetDeviceParam(request.get('RdmDestUID'),
                                       request.get('RdmParameterId'),
                                       request.get('RdmParameterData'))

    if command == RdmCommandClass.RdmSetCommand:
      setter: Callable = self.RDM_PARAMETER_SETTERS.get(parameter_id)
      if setter is not None:
        reply = setter(request.get('RdmDestUID'), request.get('RdmParameterId'),
                       request.get('RdmParameterData'))
      else:
        reply = self.RdmSetDeviceParam(request.get('RdmDestUID'),
                                       request.get('RdmParameterId'),
                                       request.get('RdmParameterData'))
    ...
    if reply is not None:
      ...
      response['RdmTransactioNumber'] = (response['RdmTransactioNumber'] +1) & 0xFF
      if command == RdmCommandClass.RdmGetCommand:
  ...
  def handle_art_address(self, op_code: OpCode, ip: str, port: int,
      reply: dict[str, Any]) -> None:
    """
    ...
    try:
      ...
      cfg = self.config
      net = cfg.get("net", 0) if reply['NetSwitch'] & 0x80 == 0 else reply[
                                                                       'NetSwitch'] & 0x7F
      sub = cfg.get("sub", 0) if reply['SubSwitch'] & 0x80 == 0 else reply[
                                                                       'SubSwitch'] & 0x7F
      universe = cfg.get("universe", 0) if reply['SwOut'][0] & 0x80 == 0 else \
        reply['SwOut'][0] & 0xF
      port_name = cfg.get("port_name", "") if len(reply['ShortName']) == 0 else \
        reply['ShortName']
      long_name = cfg.get("long_name", "") if len(reply['LongName']) == 0 else \
        reply['LongName']
