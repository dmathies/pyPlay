...
class DMXHandler:
  ...
  def dmx_receive(self, op_code, ip, port, reply):
    base_addr = self.DEVICE_INFO[0].get("parameters").get(ArtNet.rdm.RdmParameterID.RdmParamDmxStartAddress)
    ...
    if reply.get("Universe") == universe:
      ...
      event_data = {"dimmer": reply.get('Data')[base_addr-1] / 255.0,
                    "video_index": reply.get('Data')[base_addr],
                    "video_mode": reply.get('Data')[base_addr+1],
                    "fade_time": fade_time,
                    "scale": [reply.get('Data')[base_addr+3] / 128.0, reply.get('Data')[base_addr+4] / 128.0],
                    "rotation": reply.get('Data')[base_addr+5] / 255.0 * np.pi * 2,
                    "offset": [(reply.get('Data')[base_addr+6] - 128.0) / 128.0, (reply.get('Data')[base_addr+7]-128) / 128.0],
