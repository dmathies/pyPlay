...
class OSCHandler:
    def __init__(self, ip="auto", rx_port=9000, tx_port = 8000, name="osc_handler"):
        ...
        self.dispatcher.map("/qplayer/remote/*", handler=self.qplayer_handler, needs_reply_address=True)
    ...
    def handle_showfile(self, client_address: [str, int], block_number: int, total_blocks: int, blob: bytes):
        ...
        if len(self.received_showfile_chunks) == block_number+1:
            ...
            reply = udp_client.SimpleUDPClient(client_address[0], self.tx_port)
            reply.send_message("/qplayer/remote/update-show-ack", [self.name, block_number])
        else:
            # Out of order receive
            self.received_showfile_chunks={}
            reply = udp_client.SimpleUDPClient(client_address[0], self.tx_port)
            reply.send_message("/qplayer/remote/update-show-nack", [self.name, block_number])

        if len(self.received_showfile_chunks) == total_blocks:
            print(f"Received chunk {block_number+1}/{total_blocks} from {client_address[0]}:{self.tx_port}")
