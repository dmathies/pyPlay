...
class CueEngine:
    ...
    def tick(self):
        ...
        for active_cue in self.active_cues:
            runtime = now - active_cue.cue_start_time
            ...
            if active_cue.media_duration.total_seconds() > 0.0:
                ...
                if active_cue.alpha == 0.0:
                    if active_cue.endLoop == False and active_cue.media_loopMode in (
                        LoopMode.LoopedInfinite,
                        LoopMode.Looped,
                    ):
                        if (
                            active_cue.media_loopMode == LoopMode.LoopedInfinite
                            or active_cue.media_loopCount > active_cue.loop_counter
                        ):
