...
class OSCTester:
    ...
    def __init__(
        ...
        print(f"ZZOSC Tester: ip{ip}, rx:{rx_port}, tx:{tx_port}")
        self.ip = ip
        self.broadcast_addr = get_broadcast(ip)
        if not self.broadcast_addr:
            # Fallback to direct unicast if broadcast discovery fails.
            self.broadcast_addr = ip
            print(
                f"[osc_tester] Broadcast address not found for {ip}, using unicast fallback."
            )
        self.name = name
        ...
        self.client = SimpleUDPClient(
            address=self.broadcast_addr, port=self.tx_port, allow_broadcast=True
        )
    ...
    def default_handler(self, address: str, *args):
        if self.monitor:
            print(f"[OSC RX] {address} {args}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Simple OSC test client/server.")
    parser.add_argument("--ip", default="auto", help="Local IP or 'auto' (default).")
    parser.add_argument("--rx-port", type=int, default=8000, help="Local listen port.")
    parser.add_argument("--tx-port", type=int, default=9000, help="Target send port.")
    parser.add_argument("--name", default="Video1", help="Node name argument.")
    parser.add_argument(
        "--monitor",
        action="store_true",
